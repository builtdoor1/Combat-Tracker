...
public class CombatTrackerClient implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        ...
        toggleRecordKey = KeyMappingHelper.registerKeyMapping(new KeyMapping(
                "key.combat_tracker.toggle_record",
                GLFW.GLFW_KEY_UNKNOWN, // unbound by default
                KeyMapping.Category.MISC
        ));

        // 26.x extracts HUD render state instead of drawing it, so there is no
        // Gui.render(GuiGraphics, DeltaTracker) to mix into. Fabric exposes a real
        // HUD element registry instead, which is a better hook than the mixin was:
        // ordering against vanilla elements is declared rather than implied by
        // injecting at the tail of someone else's method.
        HudElementRegistry.addLast(
                Identifier.fromNamespaceAndPath("combat_tracker", "stats_hud"),
