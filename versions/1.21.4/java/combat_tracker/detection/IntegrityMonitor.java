 ...
 *
 * <p><b>Hotbar attribution is off before 1.21.5.</b> {@code Inventory.selected} is
 * a public field with no setter, so there is nothing to intercept and no way to
 * learn which vanilla path moved the slot. What remains is comparing the slot to
 * its previous value, which reports every scroll and every number key as
 * unattributed - noise, not detection, and the kind that buries the checks that do
 * work. The use, attack and keybind checks are unchanged, as is the silent-packet
 * check below, which never depended on the setter. Nothing about how a fight is
 * measured depends on any of this.</p>
 *
...
public final class IntegrityMonitor {
    ...
    public void noteSetterCall(int newSlot, InputContext.Source source) {
        // Never called here: InventoryMixin is not registered on this version
        // because there is no setter to inject into.
    }
    ...
    public void tick(LocalPlayer player) {
        // Nothing to resolve; see the hotbar note on this class.
    }
    ...
    public void checkSlotNow() {
        // Disabled on this Minecraft version; see the hotbar note on this class.
    }
