...
public final class ReportBuilder {
    ...
    /**
     * Actions with no vanilla input behind them.
     *
     * <p>Deliberately worded as an observation rather than an accusation. A clean
     * session says so plainly; a flagged one reports what tripped and how often, and
     * says out loud that ordinary mods can do the same thing.</p>
     */
    private static void inputSection(StringBuilder s, SessionData d) {
        int total = d.flagHotbar + d.flagUse + d.flagAttack + d.flagKeybind;
        s.append("<h2>Input provenance</h2>");
        if (total == 0) {
            s.append("<div class=\"sub\">Every attack, item use and hotbar change during this session came "
                    + "from the keyboard, the mouse wheel or the server, and every keybind moved because a key "
                    + "or button physically moved. Nothing was driven by anything else.</div>");
            return;
        }
        s.append("<div class=\"cards\">");
        card(s, String.valueOf(d.flagHotbar), "hotbar switches");
        card(s, String.valueOf(d.flagUse), "item uses");
        card(s, String.valueOf(d.flagAttack), "attacks");
        card(s, String.valueOf(d.flagKeybind), "keybinds");
        s.append("</div>");
        s.append("<div class=\"sub\">These actions happened while none of the vanilla input paths were running "
                + "&mdash; not a number key, not the scroll wheel, not the server moving your hand. "
                + "<b>This is not by itself proof of cheating:</b> any mod that swaps your hotbar or triggers a "
                + "use for you looks identical from here. It does mean something other than your hands did it."
                + "<br><br>Keybind counts in particular: a controller mod drives the game by setting keybinds "
                + "rather than by sending key events, so playing on a controller produces a large, steady "
                + "keybind count with nothing wrong at all. Toggle-sprint and similar accessibility mods do the "
                + "same. Read the other three rows first."
                + "</div>");
        s.append(flagChart(d));
    }

    /** Flags over time, one row per kind, so bursts and cadence are visible. */
    private static String flagChart(SessionData d) {
        if (d.flagEvents.isEmpty()) {
            return "";
        }
        int w = W;
        int h = 182;
        ...
        s.append("<svg class=\"chart\" viewBox=\"0 0 ").append(w).append(" ").append(h)
                .append("\" width=\"100%\" style=\"background:#161a20;border:1px solid #2a2f37;")
                .append("border-radius:8px;\">");
        s.append("<text x=\"").append(ML).append("\" y=\"18\" fill=\"#9fb3c8\" font-size=\"13\" ")
                .append("font-family=\"sans-serif\">Unattributed actions over time</text>");
        String[] labels = {"hotbar", "use", "attack", "keybind"};
        String[] colors = {"#ffb454", "#29b6f6", "#ef5350", "#ab7df8"};
        long span = Math.max(1, d.endEpochMs - d.startEpochMs);
        int pw = w - ML - MR;
        for (int row = 0; row < 4; row++) {
            int y = 46 + row * 32;
            s.append("<text x=\"").append(ML - 6).append("\" y=\"").append(y + 4)
                    .append("\" fill=\"#6b7480\" font-size=\"10\" text-anchor=\"end\" ")
                    .append("font-family=\"sans-serif\">").append(labels[row]).append("</text>");
            s.append("<line x1=\"").append(ML).append("\" y1=\"").append(y).append("\" x2=\"")
                    .append(ML + pw).append("\" y2=\"").append(y).append("\" stroke=\"#21262e\"/>");
            for (SessionData.FEvent e : d.flagEvents) {
                if (e.kind != row) {
                    continue;
                }
                int x = (int) (ML + (double) (e.t - d.startEpochMs) / span * pw);
                s.append("<circle cx=\"").append(x).append("\" cy=\"").append(y)
                        .append("\" r=\"3\" fill=\"").append(colors[row]).append("\" fill-opacity=\"0.85\">")
                        .append("<title>").append(esc(offset(e.t - d.startEpochMs))).append(" : ")
                        .append(labels[row]).append("</title></circle>");
            }
        }
