 ...
 * button. This class is only ever loaded when Mod Menu is present (it queries the
 * "modmenu" entrypoint), so the mod still runs without either dependency.
 */
public class ModMenuIntegration implements ModMenuApi {
    ...
    private static void buildGeneral(ConfigBuilder builder, ConfigEntryBuilder eb, CtConfig config) {
        ...
        general.addEntry(new ButtonEntry(Component.literal("Move HUD..."),
                ...
                b -> Minecraft.getInstance().setScreen(new HudPositionScreen(Minecraft.getInstance().screen))));
    }
